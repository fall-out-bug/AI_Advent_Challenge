...
async def example_performance_comparison():
    ...
    for provider_name, model_name, external_provider in providers_to_test:
        ...
        try:
            ...
            print(f"✅ {provider_name}: {end_time - start_time:.2f}s, {result.tokens_used} tokens")
    ...
    for provider_name, result in results.items():
        if result["success"]:
            print(f"{provider_name:10}: {result['time']:6.2f}s, {result['tokens']:4d} tokens")
        else:
            print(f"{provider_name:10}: Failed - {result.get('error', 'Unknown error')}")
...
async def main():
    ...
    if not os.getenv("OPENAI_API_KEY") and not os.getenv("ANTHROPIC_API_KEY"):
        ...
        print("   Set OPENAI_API_KEY and/or ANTHROPIC_API_KEY to test external providers.")
