...
class ChadGPTDemo:
    ...
    def _print_header(self) -> None:
        ...
        print("=" * 50)
        print(f"🔑 ChadGPT API: {'✅ Available' if self.api_key_available else '❌ Not set'}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if resp_json['is_success']:
                    ...
                    issues_count, recommendations_count = self._parse_review_details(review_response)
                    
                    self._record_successful_task(task, quality_score, issues_count, recommendations_count)
        ...
        try:
            ...
            for line in lines:
                if 'issues:' in line.lower():
                    ...
                    issues_count = len([l for l in lines if l.strip().startswith(('-', '•', '*', '1.', '2.', '3.'))])
                elif 'recommendations:' in line.lower():
                    recommendations_count = len([l for l in lines if l.strip().startswith(('-', '•', '*', '1.', '2.', '3.'))])
    ...
    def _record_successful_task(self, task: Dict, quality_score: int, issues_count: int = 0, recommendations_count: int = 0) -> None:
    ...
    def _print_summary(self) -> None:
        ...
        if successful_tasks:
            avg_quality = sum(r['quality_score'] for r in successful_tasks) / len(successful_tasks)
            avg_issues = sum(r['issues_count'] for r in successful_tasks) / len(successful_tasks)
            avg_recommendations = sum(r['recommendations_count'] for r in successful_tasks) / len(successful_tasks)
            ...
            for complexity, tasks in complexity_stats.items():
                ...
                print(f"   {complexity.capitalize()}: {len(tasks)} tasks, avg quality {avg_quality:.1f}/10")
