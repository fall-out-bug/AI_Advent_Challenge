...
async def demo_smart_model_selection():
    ...
    print(f"⚡ Speed preference: {speed_rec.model} (confidence: {speed_rec.confidence:.2f})")
    ...
    print(f"🎯 Quality preference: {quality_rec.model} (confidence: {quality_rec.confidence:.2f})")
    print(f"   Reasoning: {quality_rec.reasoning}")
    
    # Balanced (default)
    balanced_rec = selector.recommend_model(complex_task, "python")
    print(f"⚖️  Balanced: {balanced_rec.model} (confidence: {balanced_rec.confidence:.2f})")
