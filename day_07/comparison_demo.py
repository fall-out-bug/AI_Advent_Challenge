...
async def demo_smart_selection_comparison():
    ...
    for task in tasks:
        ...
        print(f"{task[:39]:<40} {gen_rec.model:<15} {rev_rec.model:<15} {gen_rec.confidence:.2f}")
...
async def demo_task_comparison(task_info, task_num):
    ...
    try:
        ...
        print(f"\n✅ Expected vs Actual:")
        print(f"   Generator: {expected_gen} {'✅' if gen_match else '❌'} → {gen_rec.model}")
        print(f"   Reviewer: {expected_rev} {'✅' if rev_match else '❌'} → {rev_rec.model}")
...
async def demo_workflow_comparison():
    ...
    try:
        ...
        gen_rec = generator.get_smart_model_recommendation(task, "python")
        rev_rec = reviewer.get_smart_model_recommendation(
            "Review generated web application code for security and best practices", "python"
        )
        ...
        print(f"   Quality score: {rev_result.code_quality_score}/10")
        print(f"   Security issues: {len([i for i in rev_result.issues if 'security' in i.lower()])}")
        print(f"   Performance issues: {len([i for i in rev_result.issues if 'performance' in i.lower()])}")
        ...
        print(f"   Total time: {total_time:.2f}s")
        print(f"   Total tokens: {total_tokens}")
        print(f"   Generation rate: {len(gen_result.generated_code)/gen_result.processing_time:.1f} chars/sec")
        print(f"   Review rate: {len(rev_result.review_result)/rev_result.processing_time:.1f} chars/sec")
