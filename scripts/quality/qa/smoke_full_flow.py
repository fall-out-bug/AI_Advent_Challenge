...
async def test_step_3_worker_method():
    ...
    try:
        ...
        if text:
            ...
            elif "Debug Summary" in text and ("Tasks:" in text or len([line for line in text.split('\n') if line.strip()]) > 3):
...
async def main():
    ...
    if results.get("step1_db") and results.get("step3_worker") and "No tasks found" not in str(results.get("step3_worker", "")):
