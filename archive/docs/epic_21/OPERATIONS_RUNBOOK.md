...
| `rag_rerank_score_variance` | Histogram | Score variance across chunks (per run) | >0.2 std dev |
...
### Issue: High Score Variance (>0.2 stddev per run)

**Symptoms**:
- `rag_rerank_score_variance` >0.2 for a given query (per run across chunks)
...
2. Improve prompt clarity (add examples, tie-breakers)
3. If acceptable: Document variance as expected behavior (<0.15 stddev OK)
