...
class SmartModelSelector:
    ...
    def _determine_task_type(self, task_lower: str) -> TaskType:
        ...
        if any(
            word in task_lower for word in ["review", "analyze", "check", "evaluate"]
        ):
            ...
            word in task_lower for word in ["document", "comment", "explain", "readme"]
        ):
            return TaskType.DOCUMENTATION
        elif any(
            word in task_lower for word in ["test", "unit test", "pytest", "testing"]
        ):
            return TaskType.TESTING
        elif any(
            word in task_lower
            for word in ["analyze", "analysis", "metrics", "statistics"]
        ):
    ...
    def _calculate_model_score(
        ...
        if "async" in special_reqs and "async" in capabilities.get("strengths", []):
            score += 0.1
        if "error_handling" in special_reqs and "reasoning" in capabilities.get(
            "strengths", []
        ):
            score += 0.1
    ...
    def get_all_recommendations(
        ...
        for model in self.model_capabilities.keys():
            ...
            score = self._calculate_model_score(
                model, capabilities, analysis, False, False
            )
