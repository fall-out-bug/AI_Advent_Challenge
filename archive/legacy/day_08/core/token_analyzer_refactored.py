...
class DefaultModelConfigProvider(ModelConfigProvider):
    ...
    def get_model_limits(self, model_name: str, profile: LimitProfile) -> ModelLimits:
        ...
        if model_name not in self._model_limits:
            self.logger.warning(
                f"Unknown model: {model_name}, using starcoder defaults"
            )
            model_name = "starcoder"
...
class LimitChecker:
    ...
    def check_limit_exceeded(
        ...
        try:
            ...
            self.logger.debug(
                f"Token count {token_count} vs limit {limits.max_tokens}: {'exceeded' if exceeded else 'ok'}"
            )
            return exceeded
    ...
    def get_limit_info(
        self, model_name: str, profile: LimitProfile = LimitProfile.PRACTICAL
    ) -> ModelLimits:
        """
...
class TokenCounterFacade:
    ...
    def count_tokens(self, text: str, model_name: str = "starcoder") -> TokenInfo:
        ...
        try:
            ...
            # Get model limits
            limits = self.config_provider.get_model_limits(
                model_name, self.limit_profile
            )
            ...
            return TokenInfo(
                ...
                model_name=model_name,
                exceeds_limit=False,
                limit_info=self.config_provider.get_model_limits(
                    model_name, self.limit_profile
                ),
    ...
    def estimate_compression_target(
        self, text: str, model_name: str, safety_margin: float = 0.1
    ) -> int:
        """
    ...
    def _estimate_compression_target(
        ...
        try:
            ...
            self.logger.debug(
                f"Compression target for {model_name}: {target_tokens} tokens"
            )
...
class SimpleTokenCounter(TokenCounterFacade):
    ...
    def __init__(self, limit_profile: LimitProfile = LimitProfile.PRACTICAL):
        ...
        super().__init__(
            limit_profile=limit_profile, token_calculator=SimpleTokenCalculator()
        )
...
class TokenCounter(TokenCounterFacade):
    ...
    def __init__(
        ...
        super().__init__(limit_profile=limit_profile, token_calculator=token_calculator)
