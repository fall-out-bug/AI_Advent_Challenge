...
class TestBaselineBehavior:
    ...
    def test_token_counting_baseline(self):
        ...
        long_text = (
            "This is a test text with multiple words to verify token counting accuracy"
        )
    ...
    def test_model_limits_baseline(self):
        ...
        counter = SimpleTokenCounter(
            config=MockConfiguration(), limit_profile=LimitProfile.PRACTICAL
        )
    ...
    def test_compression_ratio_baseline(self):
        ...
        truncation_result = compressor.compress_by_truncation(
            long_text, 1000, "starcoder"
        )
    ...
    def test_limit_profile_switching(self):
        ...
        # Theoretical profile
        theoretical_counter = SimpleTokenCounter(
            config=MockConfiguration(), limit_profile=LimitProfile.THEORETICAL
        )
        theoretical_limits = theoretical_counter.get_model_limits("starcoder")
        assert theoretical_limits.max_input_tokens == 16384

        # Practical profile
        practical_counter = SimpleTokenCounter(
            config=MockConfiguration(), limit_profile=LimitProfile.PRACTICAL
        )
    ...
    async def test_full_experiment_baseline(self):
        ...
        compression_result = compressor.compress_by_truncation(
            long_query, 1000, "starcoder"
        )
