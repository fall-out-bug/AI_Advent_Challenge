...
class TestTokenAnalysisClient:
    ...
    async def test_count_tokens_batch_success(self):
        ...
        with patch.object(self.client, "_execute_batch_request") as mock_execute:
            ...
            result = await self.client.count_tokens_batch(
                ["text1", "text2"], "starcoder"
            )
    ...
    async def test_compress_text_success(self):
        ...
        with patch.object(self.client, "_execute_compression_request") as mock_execute:
            ...
            result = await self.client.compress_text(
                "long text", 100, "starcoder", "truncation"
            )
...
class TestHybridTokenCounter:
    ...
    def setup_method(self):
        ...
        self.hybrid_counter = HybridTokenCounter(
            ml_client=self.mock_ml_client, fallback_counter=self.mock_fallback_counter
        )
    ...
    async def test_service_available_uses_ml_client(self):
        ...
        assert result.count == 10
        self.mock_ml_client.count_tokens.assert_called_once_with(
            "test text", "starcoder"
        )
    ...
    async def test_service_unavailable_uses_fallback(self):
        ...
        self.mock_fallback_counter.count_tokens.assert_called_once_with(
            "test text", "starcoder"
        )
    ...
    async def test_compress_text_extractive_fallback(self):
        ...
        with patch(
            "core.text_compressor.SimpleTextCompressor"
            ...
            result = await self.hybrid_counter.compress_text(
                "text", 10, "starcoder", "extractive"
            )

            assert result.strategy_used == "keywords"
            mock_compressor.compress_by_keywords.assert_called_once_with(
                "text", 10, "starcoder"
            )
    ...
    async def test_compress_text_default_fallback(self):
        ...
        with patch(
            ...
            result = await self.hybrid_counter.compress_text(
                "text", 10, "starcoder", "semantic"
            )

            assert result.strategy_used == "truncation"
            mock_compressor.compress_by_truncation.assert_called_once_with(
                "text", 10, "starcoder"
            )
