...
class TokenCountRequestModel(BaseModel):
    ...
    model_name: str = Field(
        default="starcoder", description="Model name for token counting"
    )
...
class BatchTokenCountRequestModel(BaseModel):
    ...
    texts: List[str] = Field(
        ..., min_length=1, description="List of texts to count tokens for"
    )
    model_name: str = Field(
        default="starcoder", description="Model name for token counting"
    )
...
class BatchTokenCountResponseModel(BaseModel):
    ...
    results: List[TokenCountResponseModel] = Field(
        ..., description="Results for each text"
    )
...
class TokenServiceHandler:
    ...
    async def _handle_count_tokens(
        self, request: TokenCountRequestModel
        ...
        try:
            # Validate request
            self.validator.validate_token_count_request(
                request.text, request.model_name
            )

            # Convert to service request
            service_request = TokenCountRequest(
                text=request.text, model_name=request.model_name
            )
    ...
    async def _handle_count_tokens_batch(
        self, request: BatchTokenCountRequestModel
    ) -> BatchTokenCountResponseModel:
        """
        ...
        try:
            # Validate request
            self.validator.validate_batch_token_count_request(
                request.texts, request.model_name
            )
    ...
    async def _handle_compress_text(
        self, request: CompressionRequestModel
    ) -> CompressionResponseModel:
        """
        ...
        try:
            ...
            self.validator.validate_compression_request(
                request.text, request.max_tokens, request.model_name, request.strategy
            )
    ...
    async def _handle_preview_compression(
        self, request: CompressionRequestModel
    ) -> Dict[str, Any]:
        """
