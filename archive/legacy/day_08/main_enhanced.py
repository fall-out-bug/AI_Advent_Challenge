...
async def main():
    ...
    if ml_service_available and token_mode == "hybrid":
        ...
        text_compressor = AdvancedTextCompressor(
            token_counter, None
        )  # No model_client for now
    else:
        ...
        if choice == "2":
            ...
            for text in test_texts:
                ...
                if ml_service_available:
                    try:
                        accurate_result = await token_counter.count_tokens(
                            text, "starcoder"
                        )
                        print(f"   Точный подсчет: {accurate_result.count} токенов")
                        print(
                            f"   Разница: {abs(accurate_result.count - simple_result.count)} токенов"
                        )
                    except Exception as e:
