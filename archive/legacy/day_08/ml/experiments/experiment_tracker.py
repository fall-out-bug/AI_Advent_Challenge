...
class ExperimentConfig(BaseModel):
    ...
    data_config: Dict[str, Any] = Field(
        default_factory=dict, description="Data configuration"
    )
...
class ExperimentRun(BaseModel):
    ...
    # Run metrics
    metrics: List[ExperimentMetrics] = Field(
        default_factory=list, description="Run metrics"
    )

    # Run artifacts
    artifacts: List[ExperimentArtifact] = Field(
        default_factory=list, description="Run artifacts"
    )
    ...
    # Run results
    best_metrics: Optional[ExperimentMetrics] = Field(
        default=None, description="Best metrics"
    )
    final_model_path: Optional[str] = Field(
        default=None, description="Path to final model"
    )
...
class ExperimentComparison(BaseModel):
    ...
    best_experiment: Optional[str] = Field(
        default=None, description="Best experiment ID"
    )
...
class ExperimentTracker:
    ...
    def _load_existing_data(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.experiments)} experiments, {sum(len(runs) for runs in self.runs.values())} runs"
            )
    ...
    def create_experiment(
        ...
        self.logger.info(
            f"Created experiment: {experiment.name} ({experiment.experiment_id})"
        )
        return experiment
    ...
    def log_metrics(
        ...
        # Update best metrics
        if (
            not run.best_metrics
            or experiment_metrics.f1_score > run.best_metrics.f1_score
        ):
    ...
    def complete_experiment_run(
        self, run_id: str, final_model_path: Optional[str] = None
    ) -> None:
        """
    ...
    def get_best_run(self, experiment_id: str) -> Optional[ExperimentRun]:
        ...
        return max(
            runs, key=lambda r: r.best_metrics.f1_score if r.best_metrics else 0.0
        )

    def compare_experiments(
        self, experiment_ids: List[str], comparison_metrics: Optional[List[str]] = None
        ...
        comparison = ExperimentComparison(
            experiment_ids=experiment_ids, comparison_metrics=comparison_metrics
        )
        ...
        if best_runs:
            best_experiment = max(
                best_runs.keys(), key=lambda exp_id: best_runs[exp_id].f1_score
            )
    ...
    def get_experiment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "experiments_by_status": {
                status: len(
                    [exp for exp in self.experiments.values() if exp.status == status]
                )
    ...
    def save_all_data(self) -> None:
        ...
        try:
            ...
            with open(experiments_file, "w", encoding="utf-8") as f:
                json.dump(
                    [exp.dict() for exp in self.experiments.values()],
                    f,
                    indent=2,
                    default=str,
                )
            ...
            with open(comparisons_file, "w", encoding="utf-8") as f:
                json.dump(
                    [comp.dict() for comp in self.comparisons.values()],
                    f,
                    indent=2,
                    default=str,
                )
