...
class SOLIDAuditor:
    ...
    def _audit_class(
        self, file_path: Path, class_node: ast.ClassDef, content: str
    ) -> None:
    ...
    def _check_srp_violations(
        ...
        active_concerns = [
            concern for concern, count in method_concerns.items() if count > 0
        ]
    ...
    def _check_ocp_violations(
        self, file_path: Path, class_node: ast.ClassDef, class_name: str
    ) -> None:
    ...
    def _check_isp_violations(
        ...
        if len(methods) > 8:
            ...
            if (
                io_methods
                and calc_methods
                and len(io_methods) + len(calc_methods) > len(methods) * 0.6
            ):
                self.violations.append(
                    Violation(
                        file_path=str(file_path),
    ...
    def generate_report(self) -> str:
        ...
        total_violations = sum(
            len(violations) for violations in violations_by_principle.values()
        )
    ...
    def get_most_problematic_classes(self, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        return sorted(class_violations.items(), key=lambda x: x[1], reverse=True)[
            :limit
        ]
