#!/usr/bin/env python3
"""
Терминальный чат с ехидным AI-дедушкой v3.0
Поддерживает режим советчика с ограничениями модели
"""
...
class DedChatV3:
    ...
    def setup(self):
        ...
        # Инициализируем режим советчика
    ...
    def print_welcome(self):
        ...
        print("👴" + "=" * 70)
        print("👴  ДОБРО ПОЖАЛОВАТЬ К ДЕДУШКЕ AI v3.0!")
        print("👴  Ехидный помощник с режимом советчика")
        print("👴" + "=" * 70)
        ...
        print("👴    'api chadgpt' - переключиться на ChadGPT API")
        print("👴    'api perplexity' - переключиться на Perplexity API")
        print("👴    'статус' - показать статус режима советчика")
        print("👴  НОВОЕ: Напишите 'дай совет' для активации режима советчика!")
    ...
    def get_user_input(self) -> str:
        ...
        try:
            ...
            advice_indicator = " [СОВЕТ]" if self.advice_mode and self.advice_mode.is_advice_mode_active() else ""
            user_input = input(f"🤔 Вы{mode_indicator}{api_indicator}{advice_indicator}: ").strip()
            return user_input
    ...
    async def run(self):
        ...
        while True:
            ...
            if user_message.lower() in ['покеда', 'пока', 'до свидания', 'выход', 'exit', 'quit']:
            ...
            elif user_message.lower() == 'статус':
                if self.advice_mode and self.advice_mode.is_advice_mode_active():
                    print(f"👴 Дедушка: {self.advice_mode.get_session_info()}")
                else:
                    print("👴 Дедушка: Режим советчика не активен. Напишите 'дай совет' для активации!")
                print()
                continue
                ...
                if new_api in get_available_apis():
                    ...
                    # Обновляем режим советчика с новым API
                    self.advice_mode = AdviceMode(self.api_key, self.current_api)
            ...
            # Проверяем, нужно ли активировать режим советчика
            if self.advice_mode.detect_advice_trigger(user_message):
                print("👴 Дедушка печатает...", end="", flush=True)
                response = await self.advice_mode.handle_advice_request(user_message)
                print("\r" + " " * 30 + "\r", end="")  # Очистка строки
                self.print_ded_message(response)
                continue
            
            # Если режим советчика активен, обрабатываем как часть диалога советчика
            if self.advice_mode.is_advice_mode_active():
                print("👴 Дедушка печатает...", end="", flush=True)
                response = await self.advice_mode.handle_advice_request(user_message)
                print("\r" + " " * 30 + "\r", end="")  # Очистка строки
                self.print_ded_message(response)
