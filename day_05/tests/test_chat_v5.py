...
class TestLocalModelClient:
    ...
    async def test_chat_success(self):
        ...
        client = LocalModelClient("http://localhost:8000")
    ...
    async def test_chat_api_error(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            assert "❌ Ошибка локальной модели: 500" in result["response"]
    ...
    async def test_chat_connection_error(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            assert "❌ Ошибка подключения к локальной модели: Connection failed" in result["response"]
            assert result["input_tokens"] == 0
            assert result["response_tokens"] == 0
            assert result["total_tokens"] == 0
