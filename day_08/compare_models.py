...
from core.token_analyzer import LimitProfile, TokenCounter
...
async def main():
    ...
    for query_idx, query in enumerate(queries, 1):
        ...
        try:
            results = await experimenter.run_model_comparison_experiment(
                models=available_models, query=query, auto_swap=auto_swap
            )
    ...
    if all_results:
        ...
        for model in models:
            ...
            if model_results:
                avg_time = sum(r.response_time for r in model_results) / len(
                    model_results
                )
                avg_tokens = sum(r.total_tokens for r in model_results) / len(
                    model_results
                )
                print(
                    f"   {model}: {len(model_results)} экспериментов, "
