...
class TokenService:
    ...
    async def count_tokens(self, request: TokenCountRequest) -> TokenCountResponse:
        ...
        try:
            token_info = self.token_counter.count_tokens(request.text, request.model_name)
    ...
    async def batch_count_tokens(self, request: BatchTokenCountRequest) -> BatchTokenCountResponse:
        """
        ...
        try:
            ...
            for i, text in enumerate(request.texts):
                ...
                token_request = TokenCountRequest(text=text, model_name=request.model_name)
    ...
    async def compress_text(self, request: CompressionRequest) -> CompressionResponse:
        ...
        try:
            ...
            self.logger.info(f"Text compression completed: {response.compression_ratio:.2f} ratio")
            return response
            ...
            for strategy in strategies:
                try:
                    ...
                    results[strategy] = {
                        ...
                        "preview": self._create_preview(compression_result.compressed_text)
                    }
    ...
    async def _apply_compression_strategy(self, request: CompressionRequest) -> CompressionResult:
        """
        ...
        if request.strategy == "extractive":
            ...
            raise ValueError(f"Unsupported strategy: {request.strategy}. Available: {available_strategies}")
