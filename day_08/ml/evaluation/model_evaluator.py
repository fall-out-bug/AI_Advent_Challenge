...
class EvaluationResult(BaseModel):
    ...
    evaluation_duration: float = Field(description="Total evaluation duration in seconds")
...
class ModelEvaluator:
            ...
            if self.ground_truth_path.exists():
                with open(self.ground_truth_path, 'r', encoding='utf-8') as f:
                    ...
                    self.ground_truth_data = [
                        GroundTruthData(**item) for item in data
                    ]
                self.logger.info(f"Loaded {len(self.ground_truth_data)} ground truth samples")
            else:
    ...
    async def evaluate_model(
        ...
        eval_data = self.ground_truth_data[:sample_size] if sample_size else self.ground_truth_data
        ...
        for i, sample in enumerate(eval_data):
            try:
                ...
                predicted_tokens = await self._safe_call(token_counter_func, sample.input_text)
                
                # Get compression prediction
                compressed_text = await self._safe_call(compression_func, sample.input_text)
                ...
                quality_score = self._calculate_quality_score(
                    predicted_tokens, sample.expected_token_count,
                    predicted_compression, sample.expected_compression_ratio
                )
        ...
        metrics = self._calculate_metrics(
            predictions, processing_times, compression_ratios, quality_scores, model_version
        )
        ...
        self.logger.info(f"Evaluation completed for {model_name}: MAE={metrics.mae:.3f}, RMSE={metrics.rmse:.3f}")
    ...
    def _calculate_quality_score(
        ...
        compression_error = abs(predicted_compression - expected_compression) / max(expected_compression, 0.01)
    ...
    def _calculate_metrics(
        ...
        token_percentage_errors = [
            abs(p['predicted_tokens'] - p['expected_tokens']) / max(p['expected_tokens'], 1)
            for p in predictions
        ]
        
        mae = statistics.mean(token_errors) if token_errors else 0.0
        rmse = statistics.sqrt(statistics.mean([e**2 for e in token_errors])) if token_errors else 0.0
        mape = statistics.mean(token_percentage_errors) * 100 if token_percentage_errors else 0.0
        
        # Performance metrics
        avg_processing_time = statistics.mean(processing_times) if processing_times else 0.0
        p50_processing_time = statistics.median(processing_times) if processing_times else 0.0
        p95_processing_time = self._percentile(processing_times, 95) if processing_times else 0.0
        p99_processing_time = self._percentile(processing_times, 99) if processing_times else 0.0
        ...
        # Token counting accuracy
        token_count_accuracy = statistics.mean(quality_scores) if quality_scores else 0.0
    ...
    def get_evaluation_summary(self, model_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "average_quality_score": statistics.mean(all_quality) if all_quality else 0.0,
    ...
    def load_evaluation_results(self, file_path: Path) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.evaluation_history)} evaluation results")
        except Exception as e:
