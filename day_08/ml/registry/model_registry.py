...
class ModelVersion(BaseModel):
    ...
    child_versions: List[str] = Field(default_factory=list, description="Child versions")
    
    # Version status
    is_current: bool = Field(default=False, description="Whether this is the current version")
    is_production: bool = Field(default=False, description="Whether this is in production")
...
class ModelRegistry:
    ...
    def __init__(self, base_path: Path = Path("model_registry")):
        ...
        self.versions: Dict[str, Dict[str, ModelVersion]] = {}  # model_id -> version -> ModelVersion
    ...
    def _create_directories(self) -> None:
        ...
        for path in [self.base_path, self.models_path, self.versions_path, 
                    self.promotions_path, self.deployments_path]:
    ...
    def _load_existing_data(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.models)} models, {sum(len(versions) for versions in self.versions.values())} versions")
    ...
    def promote_model(
        ...
        self.logger.info(f"Created promotion request {request.request_id} for {model_name} v{version}")
        return request
    ...
    def rollback_model(
        ...
        self.logger.info(f"Executed rollback for {model_name} to version {target_version}")
        return request
    ...
    def _save_model_files(self, model_id: str, model_files: List[Union[str, Path]]) -> None:
    ...
    def _save_registry_data(self) -> None:
        ...
        try:
            ...
            with open(models_file, 'w', encoding='utf-8') as f:
                json.dump([model.dict() for model in self.models.values()], f, indent=2, default=str)
            ...
            with open(promotions_file, 'w', encoding='utf-8') as f:
                json.dump([promo.dict() for promo in self.promotions.values()], f, indent=2, default=str)
    ...
    def get_registry_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_promotions": len([p for p in self.promotions.values() if p.status == "pending"]),
            "models_by_stage": {
