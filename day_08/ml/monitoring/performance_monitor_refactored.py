...
class PerformanceMetrics(BaseModel):
    ...
    # Prediction metrics
    avg_prediction_confidence: float = Field(
...
class MetricsCollector:
    ...
    def get_current_metrics(self) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            avg_prediction_confidence=statistics.mean(confidences)
            if confidences
            else 0.0,
...
class DriftDetector:
    ...
    def detect_drift(self, current_metrics: PerformanceMetrics) -> DriftDetectionResult:
        ...
        confidence_change = self._calculate_change(
            self.baseline_metrics.avg_prediction_confidence,
            current_metrics.avg_prediction_confidence,
        )
    ...
    def _generate_recommendations(
        ...
        if latency_drift and latency_change > 0:
            recommendations.append(
                "Consider optimizing model inference or increasing resources"
            )

        if accuracy_drift and accuracy_change < 0:
            recommendations.append(
                "Model accuracy has decreased - consider retraining or data validation"
            )

        if confidence_drift and confidence_change < 0:
            recommendations.append(
                "Model confidence has decreased - investigate input data quality"
            )

        if error_rate_drift and error_rate_change > 0:
            recommendations.append(
                "Error rate has increased - check model stability and input validation"
            )
...
class AlertManager:
    ...
    def check_and_generate_alert(
        self, drift_result: DriftDetectionResult
    ) -> Optional[Alert]:
        """
...
class PerformanceAnalyzer:
    ...
    def analyze_performance_trends(
        self, metrics_collector: MetricsCollector, hours: int = 24
    ) -> Dict[str, Any]:
        """
...
class PerformanceMonitor:
    ...
    async def _check_drift_and_alerts(self) -> None:
        ...
        if (
            self.metrics_collector.get_data_points_count()
            < self.drift_detector.baseline_window_size
        ):
            return
    ...
    def save_monitoring_data(self, file_path: Path) -> None:
        ...
        try:
            data = {
                "performance_history": {
                    key: list(values)
                    for key, values in self.metrics_collector.performance_history.items()
                },
                "baseline_metrics": self.drift_detector.baseline_metrics.dict()
        ...
        try:
            ...
            for key, values in data.get("performance_history", {}).items():
                if key in self.metrics_collector.performance_history:
                    self.metrics_collector.performance_history[key] = deque(
                        values, maxlen=self.metrics_collector.window_size
                    )

            # Restore baseline metrics
            if data.get("baseline_metrics"):
                self.drift_detector.baseline_metrics = PerformanceMetrics(
