...
async def demo_token_counting():
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        for model_name in ["starcoder", "tinyllama"]:
            ...
            print(f"   {model_name}: {'❌ Превышен' if exceeds else '✅ В пределах'} (лимит: {limits.max_input_tokens})")
...
async def demo_text_compression():
    ...
    # Compare strategies
    print(f"\n📊 Сравнение стратегий:")
    print(f"   Обрезка: {truncation_result.compression_ratio:.2f} (лучше сохраняет контекст)")
    print(f"   Ключевые слова: {keywords_result.compression_ratio:.2f} (лучше для поиска)")
