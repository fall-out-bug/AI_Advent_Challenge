...
from config.loader import ModelLimitsLoader, ModelLimitsConfig
...
class TestAppConfig:
    ...
    def test_validation_tokens_per_word_ratio(self):
        ...
        with patch.dict('os.environ', {'TOKENS_PER_WORD_RATIO': '5.0'}):
            with pytest.raises(ValueError, match="tokens_per_word_ratio must be between"):
                AppConfig()
    ...
    def test_validation_safety_margin(self):
        ...
        with patch.dict('os.environ', {'EXPERIMENT_SAFETY_MARGIN': '2.0'}):
            with pytest.raises(ValueError, match="experiment_safety_margin must be between"):
                AppConfig()
    
    def test_validation_temperature(self):
        """Test temperature validation."""
        with patch.dict('os.environ', {'MODEL_CLIENT_TEMPERATURE': '5.0'}):
            with pytest.raises(ValueError, match="model_client_temperature must be between"):
                AppConfig()
