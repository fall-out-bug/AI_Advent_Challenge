...
class TestStatisticsProperties:
    ...
    def test_average_within_bounds(self, values: list[float]):
        ...
        tolerance = 1e-10
        assert min(values) - tolerance <= avg <= max(values) + tolerance, f"Average {avg} outside bounds [{min(values)}, {max(values)}]"
    ...
    def test_median_within_bounds(self, values: list[float]):
        ...
        assert min(values) <= median <= max(values), f"Median {median} outside bounds [{min(values)}, {max(values)}]"
    ...
    def test_percentile_within_bounds(self, values: list[float], percentile: int):
        ...
        assert min(values) <= pct <= max(values), f"Percentile {pct} outside bounds [{min(values)}, {max(values)}]"
    ...
    def test_percentile_monotonic(self, values: list[float]):
        ...
        assert pct25 <= pct50 <= pct75, f"Percentiles not monotonic: {pct25} <= {pct50} <= {pct75}"
    ...
    def test_percentile_edge_cases(self, values: list[float]):
        ...
        assert pct0 == min(values), f"0th percentile should be minimum: {pct0} != {min(values)}"
        assert pct100 == max(values), f"100th percentile should be maximum: {pct100} != {max(values)}"
    ...
    def test_statistics_handle_duplicates(self, values: list[float]):
        ...
        assert abs(avg_original - avg_duplicates) < 1e-9, f"Average should be same with duplicates: {avg_original} != {avg_duplicates}"
    ...
    def test_statistics_handle_single_value(self, values: list[float]):
        ...
        assert avg == single_value[0], f"Average of single value should be the value: {avg}"
        assert median == single_value[0], f"Median of single value should be the value: {median}"
        assert pct50 == single_value[0], f"50th percentile of single value should be the value: {pct50}"
