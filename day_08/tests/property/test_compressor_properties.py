...
class TestTextCompressorProperties:
    ...
    def test_compression_ratio_bounds(self, text: str, max_tokens: int):
        ...
        assert 0 <= result.compression_ratio <= 1.0, f"Compression ratio out of bounds: {result.compression_ratio}"
    ...
    def test_compressed_tokens_within_limit(self, text: str, max_tokens: int):
        ...
        result = self.compressor.compress_text(text, max_tokens, strategy="truncation")
        assert result.compressed_tokens <= max_tokens + 5, \
            f"Compressed tokens exceed limit: {result.compressed_tokens} > {max_tokens}" # Allow small buffer
    ...
    def test_compression_idempotent(self, text: str, max_tokens: int):
        ...
        assert result1.compression_ratio == result2.compression_ratio, "Compression should be idempotent"
    ...
    def test_compression_strategy_consistency(self, text: str, max_tokens: int):
        ...
        for strategy in strategies:
            ...
            assert 0 <= result.compression_ratio <= 2.0, f"Strategy {strategy} produced invalid ratio"
            assert result.compressed_tokens <= max_tokens + 10, f"Strategy {strategy} exceeded token limit"
            # Allow some tolerance for text length (compression might add ellipsis)
            assert len(result.compressed_text) <= len(result.original_text) + 10, f"Strategy {strategy} produced much longer text"
    ...
    def test_compression_monotonic(self, text: str, max_tokens1: int, max_tokens2: int):
        ...
        result1 = self.compressor.compress_text(text, max_tokens1, strategy="truncation")
        result2 = self.compressor.compress_text(text, max_tokens2, strategy="truncation")
        ...
        if result1.compression_ratio > 1.0 and result2.compression_ratio > 1.0:
            assert result2.compression_ratio >= result1.compression_ratio - 0.5, \
    ...
    @settings(max_examples=50)
    def test_compression_no_compression_when_not_needed(self, text: str, max_tokens: int):
        ...
        if original_tokens <= max_tokens:
            result = self.compressor.compress_text(text, max_tokens, strategy="truncation")
            assert result.compression_ratio == 1.0, "No compression needed, ratio should be 1.0"
            assert result.compressed_text == result.original_text, "No compression needed, text should be unchanged"
    ...
    def test_compression_handles_special_characters(self, text: str, max_tokens: int):
        ...
        special_text = text + " !@#$%^&*()_+-=[]{}|;':\",./<>?"
        result = self.compressor.compress_text(special_text, max_tokens, strategy="truncation")
        
        assert 0 <= result.compression_ratio <= 1.0, "Compression should handle special characters"
        assert result.compressed_tokens <= max_tokens + 10, "Compression should respect token limit with special characters"

    @given(
        text=text(min_size=100, max_size=10000, alphabet="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,!?"),
        max_tokens=integers(min_value=10, max_value=1000)
    )
    @settings(max_examples=50)
    def test_compression_preserves_sentence_structure(self, text: str, max_tokens: int):
        ...
        if len(result.compressed_text) > 0 and any(c.isalpha() for c in result.compressed_text):
            ...
            if result.compression_ratio < 1.0 and len(result.compressed_text) > 20:
                assert last_char in ".!?…", f"Compressed text should end with proper punctuation, got '{last_char}'"
    ...
    def test_compression_keywords_strategy(self, text: str, max_tokens: int):
        ...
        assert 0 <= result.compression_ratio <= 1.0, "Keywords strategy should produce valid ratio"
        assert result.compressed_tokens <= max_tokens, "Keywords strategy should respect token limit"
        assert len(result.compressed_text) <= len(result.original_text), "Keywords strategy should not increase text length"
        
        # Keywords strategy should produce shorter text than truncation
        truncation_result = self.compressor.compress_text(text, max_tokens, strategy="truncation")
        assert len(result.compressed_text) <= len(truncation_result.compressed_text), \
            "Keywords strategy should produce shorter text than truncation"
