...
class TestTokenAnalyzerProperties:
    ...
    def test_token_count_non_negative(self, text: str):
        ...
        assert result.count >= 0, f"Token count should be non-negative, got {result.count}"
    ...
    def test_token_count_proportional_to_length(self, text: str):
        ...
        assert result.count <= word_count * 3, f"Token count too high: {result.count} > {word_count * 3}"
        assert result.count >= max(1, word_count // 3), f"Token count too low: {result.count} < {word_count // 3}"
    ...
    @given(
        text1=text(min_size=1, max_size=1000),
        text2=text(min_size=1, max_size=1000)
    )
    @settings(max_examples=50)
    def test_token_count_additive(self, text1: str, text2: str):
        ...
        assert combined_result.count <= individual_sum + 5, f"Combined count too high: {combined_result.count} > {individual_sum + 5}"
        assert combined_result.count >= individual_sum - 5, f"Combined count too low: {combined_result.count} < {individual_sum - 5}"
    ...
    def test_token_count_case_sensitive(self, text: str):
        ...
        ratio = abs(upper_result.count - lower_result.count) / max(upper_result.count, lower_result.count, 1)
        assert ratio <= 0.5, f"Case change caused dramatic token count change: {ratio}"

    @given(
        text=text(min_size=1, max_size=1000),
        model=one_of(st.just("starcoder"), st.just("mistral"), st.just("qwen"), st.just("tinyllama"))
    )
    @settings(max_examples=50)
    def test_token_count_model_consistent(self, text: str, model: str):
        ...
        assert result.count >= 0, f"Token count should be non-negative for model {model}"
        assert result.count <= len(text) * 2, f"Token count too high for model {model}: {result.count}"
    ...
    def test_token_count_whitespace_insensitive(self, text: str):
        ...
        if result1.count > 0 and result2.count > 0:
            ratio = abs(result1.count - result2.count) / max(result1.count, result2.count)
            assert ratio <= 0.2, f"Whitespace normalization caused significant change: {ratio}"
    ...
    def test_token_count_empty_string_handling(self, text: str):
        ...
        if not text.strip():
            result = self.counter.count_tokens(text, "starcoder")
            assert result.count == 0, f"Empty string should return 0 tokens, got {result.count}"
