...
class ConsoleFormatter:
    ...
    def format_experiment_summary(self, results: List[ExperimentResult]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            if result.compression_result:
                ...
                output.append(
                    f"   Стратегия сжатия: {result.compression_result.strategy_used}"
                )

            output.append(f"   Длина ответа: {len(result.response)} символов")
            output.append(
                f"   Время выполнения: {result.timestamp.strftime('%H:%M:%S')}"
            )
    ...
    def format_compression_comparison(self, stats: CompressionStats) -> str:
        ...
        for strategy, strategy_data in stats.strategy_stats.items():
            ...
            output.append(
                f"   Средний коэффициент сжатия: {strategy_data['avg_ratio']:.2f}"
            )
    ...
    def format_performance_metrics(self, stats: PerformanceStats) -> str:
        ...
        if stats.model_performance:
            ...
            for model_name, model_data in stats.model_performance.items():
                output.append(f"\n🔧 Модель: {model_name}")
                output.append(
                    f"   Количество экспериментов: {model_data['experiment_count']}"
                )
                output.append(
                    f"   Среднее время ответа: {model_data['avg_response_time']:.2f} сек"
                )
                output.append(
                    f"   Средние входные токены: {model_data['avg_input_tokens']:.0f}"
                )
                output.append(
                    f"   Средние выходные токены: {model_data['avg_output_tokens']:.0f}"
                )
                output.append(
                    f"   Скорость генерации: {model_data['generation_speed']:.1f} токенов/сек"
                )
    ...
    def format_recommendations(self, results: List[ExperimentResult]) -> str:
        ...
        if compressed_results:
            ...
            output.append(
                f"   Стратегия: {best_compression.compression_result.strategy_used}"
            )
    ...
    def format_detailed_analysis(
        self, stats: ExperimentStats, compression_stats: CompressionStats
    ) -> str:
        """
        ...
        if compression_stats.compression_count > 0:
            output.append(f"\n🗜️  Анализ сжатия:")
            output.append(
                f"   Экспериментов со сжатием: {compression_stats.compression_count}"
            )
            output.append(
                f"   Средний коэффициент сжатия: {compression_stats.avg_compression_ratio:.2f}"
            )
