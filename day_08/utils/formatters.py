...
class TextFormatter:
    ...
    def format_experiment_result(self, result: ExperimentResult) -> str:
        ...
        if result.estimated_cost > 0:
            lines.append(f"Estimated Cost: ${self.format_number(result.estimated_cost)}")
        
        if result.compression_result:
            comp = result.compression_result
            lines.append(f"Compression Strategy: {comp.strategy_used}")
            lines.append(f"Compression Ratio: {self.format_percentage(comp.compression_ratio)}")
            lines.append(f"Tokens Saved: {comp.original_tokens - comp.compressed_tokens}")
    ...
    def format_compression_result(self, result: CompressionResult) -> str:
        ...
        lines.append(f"Compression Ratio: {self.format_percentage(result.compression_ratio)}")
        lines.append(f"Tokens Saved: {result.original_tokens - result.compressed_tokens}")
...
class ConsoleFormatter:
    ...
    def _init_colors(self) -> Dict[str, str]:
        ...
        if not self.config.use_colors:
            return {key: "" for key in ["reset", "bold", "red", "green", "yellow", "blue", "cyan", "magenta"]}
    ...
    def format_experiment_summary(self, results: List[ExperimentResult]) -> str:
        ...
        if results:
            ...
            lines.append(self.format_key_value("Success Rate", self.format_percentage(success_rate)))
            
            avg_time = sum(r.response_time for r in results if r.response_time is not None) / len(results)
            lines.append(self.format_key_value("Average Response Time", self.formatter.format_duration(avg_time)))
    ...
    def format_compression_summary(self, results: List[CompressionResult]) -> str:
        ...
        lines.append(self.format_key_value("Average Compression Ratio", self.format_percentage(avg_ratio)))
        ...
        for strategy, strategy_results in strategies.items():
            strategy_avg = sum(r.compression_ratio for r in strategy_results) / len(strategy_results)
            lines.append(self.format_key_value(f"{strategy.title()} Strategy", 
    ...
    def format_recommendations(self, results: List[ExperimentResult]) -> str:
        ...
        if len(failed) > len(successful):
            lines.append(self.format_warning("⚠️  High failure rate detected. Consider:"))
        ...
        compression_results = [r.compression_result for r in results if r.compression_result]
        if compression_results:
            avg_ratio = sum(r.compression_ratio for r in compression_results) / len(compression_results)
            if avg_ratio < 0.5:
                lines.append(self.format_info("💡 Good compression achieved. Consider:"))
        ...
        if response_times:
            ...
            if avg_time > 10:
                lines.append(self.format_warning("⚠️  Slow response times detected. Consider:"))
