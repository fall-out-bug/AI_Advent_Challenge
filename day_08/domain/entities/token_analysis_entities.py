...
class TokenAnalysisDomain:
    ...
    def get_quality_score(self) -> float:
        ...
        time_factor = max(0.0, 1.0 - (self.processing_time or 0.0) / 10.0)  # Penalize slow processing
    ...
    def validate(self) -> List[str]:
        ...
        if self.is_completed():
            ...
            if self.compression_ratio is None or not (0.0 <= self.compression_ratio <= 1.0):
            ...
            if self.confidence_score is None or not (0.0 <= self.confidence_score <= 1.0):
...
class CompressionJob:
    ...
    def validate(self) -> List[str]:
        ...
        if self.is_completed():
            ...
            if self.actual_compression_ratio is None or not (0.0 <= self.actual_compression_ratio <= 1.0):
...
class ExperimentSession:
    ...
    def add_result(self, analysis: TokenAnalysisDomain) -> None:
        ...
        if self.status != "running":
            raise ValueError(f"Cannot add results to session with status: {self.status}")
