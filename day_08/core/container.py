...
from core.advanced_compressor import ExtractiveCompressor, SemanticChunker
...
class ApplicationContainer(containers.DeclarativeContainer):
    ...
    token_counter = providers.Factory(
        TokenCounterFactory.create_simple,
    ...
    ml_client = providers.Singleton(
        TokenAnalysisClient,
        base_url=config.ml_service.url
    )
...
def configure_container(config_dict: Optional[Dict[str, Any]] = None) -> ApplicationContainer:
    """
...
def get_text_compressor(
    text_compressor: SimpleTextCompressor = Provide[ApplicationContainer.text_compressor]
) -> SimpleTextCompressor:
...
def get_application_context(
    context = Provide[ApplicationContainer.application_context]
):
...
def initialize_container(config_dict: Optional[Dict[str, Any]] = None) -> ApplicationContainer:
    """
...
def check_container_health(container_instance: Optional[ApplicationContainer] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        health_status["components"]["token_counter"] = container_to_check.token_counter() is not None
        health_status["components"]["text_compressor"] = container_to_check.text_compressor() is not None
        health_status["components"]["ml_client"] = container_to_check.ml_client() is not None
        health_status["components"]["experiments"] = container_to_check.experiments() is not None
        health_status["components"]["token_service"] = container_to_check.token_service() is not None
