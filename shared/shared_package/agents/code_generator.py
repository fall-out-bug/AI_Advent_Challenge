...
class CodeGeneratorAgent(BaseAgent):
    ...
    async def _process_impl(self, request: AgentRequest) -> AgentResponse:
        ...
        metadata = self._create_metadata(
            task_type="code_generation", model_name=self.model_name
        )
    ...
    def _build_prompt(self, request: AgentRequest) -> str:
        ...
        """
        language = (
            request.context.get("language", "python") if request.context else "python"
        )
...
Return only the code without explanations:"""
    ...
    def set_parameters(
        ...
        if temperature is not None:
            self.temperature = temperature
        logger.info(
            "CodeGeneratorAgent parameters updated: max_tokens=%s, temperature=%s",
            self.max_tokens,
            self.temperature,
        )
