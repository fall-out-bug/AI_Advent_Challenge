...
class CodeReviewerAgent(BaseAgent):
    ...
    async def _process_impl(self, request: AgentRequest) -> AgentResponse:
        ...
        metadata = self._create_metadata(
            task_type="code_review", model_name=self.model_name
        )
        ...
        if (
            len(
                [
                    line
                    for line in lines
                    if "def " in line and len(line.split("def ")[1].split("(")[0]) < 3
                ]
            )
            > 0
        ):
    ...
    def set_parameters(
        ...
        if temperature is not None:
            self.temperature = temperature
        logger.info(
            "CodeReviewerAgent parameters updated: max_tokens=%s, temperature=%s",
            self.max_tokens,
            self.temperature,
        )
