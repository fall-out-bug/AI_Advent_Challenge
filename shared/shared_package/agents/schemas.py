...
class TaskMetadata(BaseModel):
    ...
    task_type: str = Field(
        ..., description="Type of task (e.g., 'code_generation', 'code_review')"
    )
...
class AgentRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None, description="Additional context for the task"
    )
...
class QualityMetrics(BaseModel):
    ...
    readability: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Readability score"
    )
    efficiency: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Efficiency score"
    )
    correctness: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Correctness score"
    )
    maintainability: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Maintainability score"
    )
...
class AgentResponse(BaseModel):
    ...
    quality: Optional[QualityMetrics] = Field(
        None, description="Quality metrics (for review tasks)"
    )
