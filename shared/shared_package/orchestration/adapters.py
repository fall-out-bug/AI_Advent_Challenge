...
class AdapterConfig(BaseModel):
    ...
    timeout: float = Field(30.0, ge=0.1, le=300.0, description="Request timeout in seconds")
    ...
    async def send_request(
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                last_error = e
                self.logger.warning(f"REST request attempt {attempt + 1} failed: {str(e)}")
                
                if attempt < self.config.max_retries - 1:
                    await asyncio.sleep(self.config.retry_delay)
        
        self.logger.error(f"REST request to {agent_id} failed after {self.config.max_retries} attempts")
