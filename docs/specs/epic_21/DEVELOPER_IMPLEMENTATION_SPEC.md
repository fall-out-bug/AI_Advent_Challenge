...
- Seed controls: optional `seed` in config; env override `RAG_RERANK_TEMPERATURE` for debug.
...
- Metrics (`src/infrastructure/metrics/rag_metrics.py`):
  ...
  - Histogram `rag_rerank_score_variance` (variance across repeated runs)
- Unit tests:
...
- YAML: `config/retrieval_rerank_config.yaml`:
  ...
  - `reranker.enabled`, `reranker.strategy`, `reranker.llm.temperature: 0.5`, `seed`, `temperature_override_env`
...
- Seed control supported for experiments; env override recognized.
