...
async def add_channel(
    ...
    if existing:
        ...
        if title is None or description is None:
            try:
                logger.info(
                    f"Fetching fresh metadata for existing channel: {channel_username}"
                )
                ...
                if metadata.get("success"):
                    ...
                    if (
                        final_description is not None
                        and final_description != existing.get("description")
                    ):
                    ...
                    if update_fields:
                        ...
                        logger.info(
                            f"Updated existing channel metadata: {channel_username}, fields={update_fields.keys()}"
                        )
            except Exception as e:
                logger.warning(
                    f"Failed to fetch metadata for existing channel {channel_username}: {e}"
                )
    ...
    try:
        ...
        logger.error(f"Error validating channel {channel_username}: {e}", exc_info=True)
...
async def list_channels(user_id: int, limit: int = 100) -> Dict[str, Any]:
    ...
    for ch in channels:
        ...
        logger.info(
            f"Processing channel {channel_username}, has_title={bool(current_title)}, title={current_title}"
        )
        ...
        if not current_title or not current_title.strip():
            if channel_username:
                try:
                    logger.info(
                        f"Channel {channel_username} missing title, calling get_channel_metadata..."
                    )
                    metadata = await get_channel_metadata(
                        channel_username, user_id=user_id
                    )
                    logger.info(
                        f"get_channel_metadata result for {channel_username}: success={metadata.get('success')}, title={metadata.get('title')}"
                    )
                    if metadata.get("success") and metadata.get("title"):
                        ...
                        logger.info(
                            f"Updated channel {channel_username} in DB with metadata: title={new_title}, modified={update_result.modified_count}"
                        )
                    else:
                        logger.warning(
                            f"get_channel_metadata failed for {channel_username}: success={metadata.get('success')}, error={metadata.get('error')}"
                        )
                except Exception as e:
                    logger.error(
                        f"Failed to get metadata for {channel_username}: {e}",
                        exc_info=True,
                    )
                    # Continue without metadata
        else:
            logger.debug(
                f"Channel {channel_username} already has title: {current_title}"
            )
