...
from src.domain.interfaces.homework_checker import (
    HomeworkCheckerProtocol,
)
...
class HomeworkReviewServiceImpl(HomeworkReviewService):
    ...
    async def list_homeworks(self, days: int) -> Dict[str, Any]:
        ...
        """
        try:
            return await self.hw_checker.get_recent_commits(
                days=days
            )
        except Exception as e:
            logger.error(
                f"Failed to list homeworks: {e}", exc_info=True
            )
            raise HomeworkReviewError(
                f"Failed to list homeworks: {e}"
            ) from e
    ...
    async def review_homework(
        ...
        try:
            ...
            archive_bytes = await self.hw_checker.download_archive(
                commit_hash
            )
            ...
            logger.info(
                f"Saved archive to {temp_path} (size: {archive_size} bytes)"
            )

            try:
                # Call review tool
                logger.info(
                    f"Starting review for commit {commit_hash}"
                )
                ...
                if (
                    not markdown_report or not markdown_report.strip()
                ):
                ...
                try:
                    ...
                    self.storage_service.cleanup_temp_file(
                        temp_path
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to delete temp file {str(temp_path)}: {e}"
                    )
