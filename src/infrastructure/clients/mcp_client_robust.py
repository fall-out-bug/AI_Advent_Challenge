...
class RobustMCPClient:
    ...
    async def discover_tools(self) -> List[Dict[str, Any]]:
        ...
        """
        return await self._retry_call(self.base_client.discover_tools)
    ...
    async def call_tool(
        ...
        try:
            result = await self._retry_call(
                self.base_client.call_tool, tool_name, arguments
            )
    ...
    async def _retry_call(self, func, *args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error(f"Max retries exceeded: {last_exception}", exc_info=True)
    ...
    def _classify_exception(self, error: Exception) -> Exception | None:
        ...
        if any(
            word in error_str
            for word in ["connection", "connect", "refused", "reset", "closed"]
        ):
