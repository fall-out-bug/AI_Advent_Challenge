...
>>>>>>> origin/master
        ...
        try:
            response = await client.post(
                chat_url, json=chat_payload, timeout=self.timeout
            )
            ...
            if e.response is not None and e.response.status_code == 404:
                ...
                if self._host_url is None and (
                    "llm-server" in self.url or "mistral" in self.url.lower()
                ):
                    self._host_url = self._get_host_url(self.url)
                    logger.info(
                        f"Trying host URL fallback after HTTP error: {self._host_url}"
                    )
                    try:
                        ...
                        if content:
                            logger.info(
                                f"Successfully connected to LLM via host URL after HTTP error"
                            )
                            return content
                    except Exception as host_error:
                        logger.debug(
                            f"Host URL fallback after HTTP error also failed: {host_error}"
                        )
                raise
            ...
            if self._host_url is None and (
                ...
                self._host_url = self._get_host_url(self.url)
                logger.info(
                    f"Trying host URL fallback after connection error: {self._host_url}"
                )
                try:
                    host_url = f"{self._host_url}/chat"
                    host_payload = {
                    ...
                    if content:
                        logger.info(
                            f"Successfully connected to LLM via host URL after connection error"
                        )
                        return content
                except Exception as host_error:
                    logger.debug(
                        f"Host URL fallback after connection error also failed: {host_error}"
                    )
...
>>>>>>> origin/master
        ...
        try:
            response = await client.post(
                openai_url, json=openai_payload, timeout=self.timeout
            )
            response.raise_for_status()
            data = response.json()
...
>>>>>>> origin/master
                    ...
                    if content:
                        logger.info(
                            f"Successfully connected to LLM via host URL: {self._host_url}"
                        )
                        return content
...
>>>>>>> origin/master
            ...
            logger.warning(
                "LLM connection error, service may be unavailable: %s",
                e,
                extra={"url": self.url},
            )
            raise
...
>>>>>>> origin/master
        ...
        summarization_keywords = [
            ...
            "news",
            "перед тобой",
            "перед тобой фрагмент",
            "перед тобой несколько",
            "ключевых фактов",
            "key facts",
            "фрагмент",
            "fragment",
        ...
        if (
            "суммар" in prompt_lower
            or "summary" in prompt_lower
            or "дайджест" in prompt_lower
            or "digest" in prompt_lower
        ):
            is_intent = False
            is_summarization = True
        ...
        if is_summarization and not is_intent:
            ...
            for line in lines:
                ...
                if (
                    not in_content_section
                    and any(skip in line_lower for skip in skip_keywords)
                    or line_stripped.startswith(
                        ("Пример", "ВАЖНО", "Example", "CRITICAL", "EXAMPLE")
                    )
                ):
                    continue
                        ...
                        if not line_stripped.lower().startswith(
                            ...
                            if post_texts:
                                for recent_post in post_texts[-3:]:
                                    if (
                                        line_stripped[:50].lower()
                                        in recent_post.lower()[:50]
                                        or recent_post.lower()[:50]
                                        in line_stripped[:50].lower()
                                    ):
                                        is_duplicate = True
                                        break
                            if not is_duplicate:
            ...
            if post_texts:
                ...
                for line in lines:
                    ...
                    if (
                        ...
                        and not any(
                            skip in line_stripped.lower() for skip in skip_keywords
                        )
                        and not line_stripped.lower().startswith(
                            (
                                "задача:",
                                "task:",
                                "пример:",
...
=======
    async def batch_generate(
        ...
        Returns:
            List of fallback responses (one per prompt).
        """
        import asyncio
