...
Return ONLY the 3-line haiku, nothing else."""
    ...
    try:
        ...
        for line in lines:
            ...
            if (line and 
                len(line) < 150 and  # Reasonable haiku line length
                not line.startswith('Generate a haiku') and
                not 'Component:' in line[:50] and
                not 'Critical issues:' in line[:50] and
                not 'Top issues:' in line[:50] and
                not 'Return ONLY' in line):
        ...
        if len(clean_lines) >= 3:
            ...
            haiku_text = '\n'.join(clean_lines) if clean_lines else "Code flows onward,\nIssues found, lessons learned—\nImprovement awaits."
        return haiku_text
    except Exception:
...
async def generate_detailed_markdown_report(
    ...
    if client:
        top_titles = [f.title[:40] for f in report.pass_1.findings[:3]] if report.pass_1 and report.pass_1.findings else []
        haiku = await _generate_haiku_from_model(
            client, critical_count, major_count,
    ...
    md_parts.append(f"- **Detected Components**: {', '.join(report.detected_components)}")
    ...
    if report.detected_components:
        ...
        md_parts.append("*No specific components detected. General analysis performed.*")
        md_parts.append("")
    ...
    if linter_results:
        ...
        if summary:
            ...
            md_parts.append(f"**Successfully Completed**: {summary.get('tools_successful', 0)}")
            md_parts.append("")
            ...
            if flake8.get("success"):
                ...
                if flake8.get("issues"):
                    ...
                    for issue in flake8["issues"][:10]:  # Top 10 issues
                        ...
                        md_parts.append(f"- `{file_path}:{issue.get('line', 'N/A')}` - {error_message}")
                md_parts.append("")
            else:
                md_parts.append(f"- **Status**: {'Installed' if flake8.get('error') != 'Not installed' else 'Not Installed'}")
                if flake8.get("error"):
        ...
        if "pylint" in tools:
            ...
            if pylint.get("success"):
                ...
                md_parts.append(f"- **Status**: {'Installed' if pylint.get('error') != 'Not installed' else 'Not Installed'}")
                if pylint.get("error"):
        ...
        if "mypy" in tools:
            ...
            if mypy.get("success"):
                ...
                md_parts.append(f"- **Status**: {'Installed' if mypy.get('error') != 'Not installed' else 'Not Installed'}")
                if mypy.get("error"):
        ...
        if "black" in tools:
            ...
            if black.get("success"):
                ...
                md_parts.append(f"- **Needs Reformatting**: {black.get('needs_reformatting', 0)}")
                md_parts.append("")
            else:
                md_parts.append(f"- **Status**: {'Installed' if black.get('error') != 'Not installed' else 'Not Installed'}")
                if black.get("error"):
        ...
        if "isort" in tools:
            ...
            if isort.get("success"):
                ...
                md_parts.append(f"- **Status**: {'Installed' if isort.get('error') != 'Not installed' else 'Not Installed'}")
                if isort.get("error"):
        ...
        if report.pass_1.findings:
            for finding in report.pass_1.findings:
                severity_emoji = {
                    ...
                    "major": "🟠",
                    "minor": "🟡",
                }.get(finding.severity.value if hasattr(finding.severity, "value") else str(finding.severity).lower(), "⚪")
        ...
        if report.pass_3.recommendations:
            ...
            for i, rec in enumerate(report.pass_3.recommendations, 1):
                ...
                if len(rec_text) > 150:
                    ...
                    if len(sentences) > 1:
                        rec_lines = [f"{s}." if not s.endswith('.') and i < len(sentences) - 1 else s 
    ...
    if review_logger:
        ...
        if reasoning_log:
            ...
            if model_responses:
                ...
                md_parts.append("")
                for i, resp in enumerate(model_responses[-3:], 1):  # Last 3 model responses
                    pass_name = resp.get("pass_name", "Unknown")
                    prompt_preview = resp.get("prompt", "")[:300] + "..." if len(resp.get("prompt", "")) > 300 else resp.get("prompt", "")
                    response_preview = resp.get("response", "")[:500] + "..." if len(resp.get("response", "")) > 500 else resp.get("response", "")
                    ...
                    if tokens:
                        md_parts.append(f"*Tokens: {tokens.get('input_tokens', 0)} input, {tokens.get('output_tokens', 0)} output*")
                        md_parts.append("")
    ...
    if review_logger:
        ...
        md_parts.append("")
        md_parts.append(f"- **Total Review Duration**: {report.execution_time_seconds:.2f} seconds")
        if total_time_ms > 0:
            md_parts.append(f"- **Model Response Time**: {total_time_ms / 1000:.2f} seconds")
        md_parts.append("")
