...
class DataHandler(Handler):
    ...
    async def handle(self, context: DialogContext, message: str) -> str:
        ...
        if self.hybrid_classifier:
            try:
                ...
                if intent_result.intent == IntentType.DATA_SUBSCRIPTION_REMOVE:
                    channel_name = intent_result.entities.get("channel_name_hint") or intent_result.entities.get("channel_username")
                    if channel_name:
                    ...
                    hours = days * 24
                    if channel_name:
                        return await self._get_channel_digest_by_name(context, channel_name, hours)
                    else:
                    ...
                    hw_keywords = [
                        "проверок домашних", "проверки домашних", "проверки домашк", 
                        "домашних заданий", "домашних работ", "домашек", "homework", "hw", "проверок",
                        "полный статус", "полный статус домашних", "статус домашних", "статус домашних работ"
                    ]
                    # More aggressive matching: check for any homework-related content
                    has_hw = any(keyword in message_lower for keyword in hw_keywords) or \
                             ("статус" in message_lower and ("домашн" in message_lower or "работ" in message_lower))
                    if has_hw:
                        logger.info(f"Redirecting DATA_STATS to DATA_HW_STATUS based on message content: {message[:100]}")
                    ...
                    job_id = submission_id.get("job_id") if isinstance(submission_id, dict) else None
                    archive_name = submission_id.get("archive_name") if isinstance(submission_id, dict) else None
                    commit_hash = submission_id.get("commit_hash") if isinstance(submission_id, dict) else None
                    ...
                    return await self._retry_hw_submission(
                        context, job_id=job_id, archive_name=archive_name, 
                        commit_hash=commit_hash, assignment=assignment
                    )
            except Exception as e:
                logger.warning(f"Hybrid classifier failed in DataHandler: {e}, falling back to rule-based parsing")
        ...
        unsubscribe_checks = [
            "отпис", "отпиш", "unsubscribe", "удал", "remove", "убери",
        ]
        ...
        list_indicators = [
            "мои подписк", "мои каналы", "my subscription", "my subscriptions",
            "list", "список", "покажи", "show", "give me", "give me all",
            "all my", "все мои"
        ]
        if any(indicator in message_lower for indicator in list_indicators):
            if any(word in message_lower for word in ["подписк", "subscription", "канал", "channels"]):
                return await self._list_channels(context)
        
        if "channel" in message_lower or "digest" in message_lower or "дайджест" in message_lower:
            channel_name = self._parse_channel_name_from_message(message)
            days = self._parse_days_from_message(message)
            hours = days * 24
            if channel_name:
        ...
        if any(keyword in message_lower for keyword in ["статус проверки домашних", "статус проверки домашк", "статус домашних заданий", "статус домашек", "статус домашк", "homework status", "hw status", "проверки домашних"]):
            return await self._get_hw_status(context)
        elif "student" in message_lower or ("stats" in message_lower and "homework" not in message_lower and "hw" not in message_lower) or "статистика" in message_lower:
            return await self._get_student_stats(context)
        elif any(keyword in message_lower for keyword in ["статус очеред", "queue status", "очередь"]):
            return await self._get_hw_queue_status(context)
        elif any(keyword in message_lower for keyword in ["перезапусти сабмит", "retry submission", "retry сабмит"]):
            ...
            if identifier:
                ...
                commit_hash = identifier.get("commit_hash")
                return await self._retry_hw_submission(
                    context, job_id=job_id, archive_name=archive_name, commit_hash=commit_hash
                )
            else:
    ...
    def _parse_channel_name_from_message(self, message: str) -> str | None:
        ...
        stop_words = {
            'за', 'на', 'по', 'дн', 'дня', 'дней', 'день', 'дней', 'days', 'day',
            'of', 'for', 'the', 'a', 'an', 'in', 'at', 'on', 'to', 'from',
            'канала', 'channel', 'channels', 'digest', 'дайджест'
        }
    ...
    async def _get_channel_digest_by_name(self, context: DialogContext, channel_name: str, hours: int) -> str:
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"Failed to get channel digest by name: {error_msg}", exc_info=True)
            
            if "connection" in error_msg.lower() or "failed" in error_msg.lower() or "http" in error_msg.lower():
    ...
    async def _get_channels_digest(self, context: DialogContext, hours: int = 168) -> str:
    ...
    def _parse_channel_name_from_subscribe(self, message: str) -> str | None:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if channel and len(channel) >= 3 and channel.lower() not in [
                    'на', 'to', 'канал', 'channel', 'подпиш', 'subscribe', 
                    'подпишись', 'подписать', 'добавь', 'add'
                ]:
                    return channel
        
        # Final fallback: find @channel pattern anywhere in message
        at_channel_match = re.search(r'@([a-zA-Zа-яА-Я0-9_]{3,})', message, re.IGNORECASE)
        if at_channel_match:
            return at_channel_match.group(1).strip()
        
        return None

    async def _subscribe_to_channel(self, context: DialogContext, channel_username: str) -> str:
        ...
        try:
            ...
            if status in ["subscribed", "already_subscribed"]:
                ...
                logger.warning(f"Unexpected status from add_channel: {status}, result: {result}")
                safe_username = _escape_markdown(channel_username)
    ...
    def _parse_channel_name_from_unsubscribe(self, message: str) -> str | None:
        ...
        for pattern in patterns:
            ...
            if match:
                channel = match.group(1).strip().lstrip('@')
                # Skip common words
                if channel and channel.lower() not in ['от', 'from', 'канал', 'channel']:
                    return channel
        return None

    async def _unsubscribe_by_name(self, context: DialogContext, channel_name_hint: str) -> str:
            ...
            for channel in channels:
                ...
                try:
                    metadata = await self.tool_client.call_tool(
                        ...
                        {"channel_username": channel_username, "user_id": int(context.user_id)}
                    )
                    ...
                    if (hint_lower in title or hint_lower in description or 
                        hint_lower in username or username.startswith(hint_lower)):
                        ...
                        if not matched_channel_id:
                            ...
                            logger.warning(f"Channel {channel_username} has no ID field")
                        break
                except Exception as e:
                    logger.debug(f"Failed to get metadata for {channel_username}: {e}")
                    # Try direct username match if metadata fetch fails
                    if channel_username.lower().startswith(hint_lower) or hint_lower in channel_username.lower():
                        matched_channel = channel
                        matched_channel_id = channel.get("id") or str(channel.get("_id", ""))
                        break
            
            # Step 4: Delete matched channel
            if matched_channel and matched_channel_id:
                logger.info(f"Attempting to delete channel: {matched_channel_id}, user: {context.user_id}")
                ...
                if status == "deleted":
                    channel_display = matched_channel.get("channel_username", channel_name_hint)
                    safe_display = _escape_markdown(channel_display)
                    return f"✅ Отписался от канала {safe_display}."
                else:
                    logger.warning(f"Unexpected delete_channel status: {status}, result: {delete_result}")
                    safe_status = _escape_markdown(status if status else 'неизвестен')
    ...
    async def _list_channels(self, context: DialogContext) -> str:
        ...
        try:
            ...
            for i, channel in enumerate(channels, 1):
                channel_username = channel.get("channel_username", channel.get("channel", "неизвестный"))
                channel_title = channel.get("title", channel.get("channel_title", ""))
                active = channel.get("active", True)
                status = "✅" if active else "⏸"
                
                logger.info(f"Channel {i}: username={channel_username}, title={channel_title}, has_title={bool(channel_title)}, full_channel={channel}")
    ...
    def _format_single_digest(self, digest: dict) -> str:
        ...
        if len(summary) > max_summary_length:
            ...
            if last_double_newline > max_summary_length * 0.7:  # If found within last 30%
    ...
    async def _get_hw_status(self, context: DialogContext) -> str:
        ...
        try:
            ...
            for check in checks[:10]:
                ...
                safe_job_id = str(job_id).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                safe_student_id = str(student_id).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                safe_assignment = str(assignment).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                safe_progress = str(progress).replace("*", "").replace("_", "").replace("[", "").replace("]", "") if progress else ""
                safe_error = str(error_message).replace("*", "").replace("_", "").replace("[", "").replace("]", "") if error_message else ""
                safe_archive = str(archive_name).replace("*", "").replace("_", "").replace("[", "").replace("]", "") if archive_name else ""
                safe_commit = str(commit_hash).replace("*", "").replace("_", "").replace("[", "").replace("]", "") if commit_hash else ""
        ...
        try:
            ...
            if jobs:
                ...
                for job in jobs[:20]:  # Show up to 20 jobs
                    job_id = str(job.get("job_id", "N/A")).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                    status = job.get("status", "unknown")
                    student_id = str(job.get("student_id", "Неизвестный")).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                    assignment = str(job.get("assignment", "N/A")).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
                    progress = str(job.get("progress", "")).replace("*", "").replace("_", "").replace("[", "").replace("]", "")
    ...
    def _parse_submission_identifier_from_message(self, message: str) -> dict:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if not is_job_id_explicit and re.match(r"^[a-f0-9]{7,40}$", identifier, re.IGNORECASE):
                ...
                if re.match(r"^[a-zA-Z0-9_-]{8,}$", identifier):
                    ...
                    if re.search(r"[A-FG-Z]", identifier) or not re.match(r"^[a-f0-9_-]+$", identifier, re.IGNORECASE):
                        return {"job_id": identifier}
