...
class TestCollectDataUseCase:
    ...
    @pytest.mark.asyncio
    async def test_get_channels_digest_with_hours(self, usecase, mock_tool_client):
        """Ensure optional hours parameter is forwarded."""
        mock_tool_client.call_tool.return_value = {"digests": []}

        await usecase.get_channels_digest(user_id=321, hours=48)

        mock_tool_client.call_tool.assert_awaited_once_with(
            "get_channel_digest", {"user_id": 321, "hours": 48}
        )
