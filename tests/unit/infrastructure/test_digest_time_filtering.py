...
def test_time_filtering_filters_by_cutoff_time(mock_posts_with_timestamps):
    ...
    filtered = [
        p
        for p in mock_posts_with_timestamps
...
def test_time_filtering_respects_custom_hours():
    ...
    filtered = [p for p in posts if datetime.fromisoformat(p["date"]) >= cutoff_time]
    ...
    with patch("src.infrastructure.clients.telegram_utils.Bot") as mock_bot:
        ...
        if posts:
            for post in posts:
                ...
                assert (
                    post_date >= since
                ), f"Post {post['message_id']} is before cutoff time"
...
async def test_get_channel_digest_calculates_cutoff_correctly():
    ...
    with patch("src.presentation.mcp.tools.digest_tools.get_db", return_value=mock_db):
        with patch(
            ...
            with patch(
                "src.presentation.mcp.tools.digest_tools.summarize_posts",
    ...
    if called_since:
        ...
        assert (
            time_diff < 60
        ), f"Expected cutoff around {expected_cutoff}, got {called_since}"
...
def test_summarize_posts_receives_filtered_posts():
    ...
    filtered = [p for p in posts if datetime.fromisoformat(p["date"]) >= cutoff]
