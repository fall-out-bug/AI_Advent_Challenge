...
@pytest.fixture
def adaptive_summarizer(mock_direct_summarizer, mock_map_reduce_summarizer, mock_token_counter):
...
@pytest.mark.asyncio
async def test_uses_direct_for_short_text(adaptive_summarizer, mock_token_counter, mock_direct_summarizer):
...
@pytest.mark.asyncio
async def test_uses_map_reduce_for_long_text(adaptive_summarizer, mock_token_counter, mock_map_reduce_summarizer):
...
@pytest.mark.asyncio
async def test_uses_direct_at_threshold(adaptive_summarizer, mock_token_counter, mock_direct_summarizer):
...
@pytest.mark.asyncio
async def test_summarize_posts_delegates(adaptive_summarizer, mock_token_counter, mock_direct_summarizer):
    """Test summarize_posts delegates correctly."""
    from src.domain.value_objects.post_content import PostContent
    
    posts = [PostContent(text="Short post " * 10, channel_username="test")]  # Longer post
