...
def mcp_http_server():
    ...
    print(f"Starting MCP HTTP server on port {port} with DB_NAME={test_env.get('DB_NAME')}...")
...
class TestChatMode:
    ...
    async def test_chat_mode_greeting(
        ...
        assert any(word in response.lower() for word in ["привет", "здравствуй", "hello", "hi"])
...
class TestSubscriptionListing:
    ...
    async def test_list_subscriptions_request(
        ...
        assert "naboka" in response.lower() or "xor_journal" in response.lower() or "1." in response
...
class TestChannelSubscription:
    ...
    async def test_subscribe_to_channel(
        ...
        assert "subscribed" in response.lower() or "подписк" in response.lower() or "добав" in response.lower()
...
class TestChannelUnsubscription:
    ...
    async def test_unsubscribe_from_channel_by_name(
        ...
        assert channel["active"] is False, f"Channel still active. Response was: {response}"
        assert "отпис" in response.lower() or "unsubscribed" in response.lower() or "удален" in response.lower()
...
class TestDigestGeneration:
    ...
    async def test_get_digest_with_channel_resolution(
        ...
        posts_count = await real_mongodb.posts.count_documents({"user_id": user_id_int, "channel_username": "onaboka"})
