...
async def test_posts_sorted_old_to_new(real_mongodb):
    ...
    posts = [
        {"channel_username": "test_channel", "text": "Post 3", "date": now - timedelta(hours=1), "message_id": "3", "test_data": True},
        {"channel_username": "test_channel", "text": "Post 1", "date": now - timedelta(hours=3), "message_id": "1", "test_data": True},
        {"channel_username": "test_channel", "text": "Post 2", "date": now - timedelta(hours=2), "message_id": "2", "test_data": True},
    ]
...
async def test_summary_respects_max_chars_parameter(real_mongodb):
    ...
    saved_posts = await real_mongodb.posts.find({"channel_username": "test_summary_param"}).to_list(length=10)
