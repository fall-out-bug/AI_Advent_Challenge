...
class TestMCPTools:
    ...
    async def call_tool(
        ...
        ), f"Tool {tool_name} failed with status {response.status_code}"
        result = response.json()
    ...
    async def test_list_models(self, http_client):
        ...
        assert (
            "local_models" in result or "api_models" in result
        ), "Should contain model lists"
    ...
    async def test_check_model(self, http_client):
        ...
        result = await self.call_tool(
            http_client, "check_model", {"model_name": "mistral"}
        )
    ...
    async def test_count_tokens(self, http_client):
        ...
        """
        result = await self.call_tool(
            http_client, "count_tokens", {"text": "Hello world test"}
        )
    ...
    async def test_generate_code_simple(self, http_client):
        ...
        assert isinstance(result, dict), "Result should be a dictionary"
        assert (
            "success" in result or "code" in result
        ), "Should indicate success or contain code"
    ...
    async def test_generate_code_complex(self, http_client):
        ...
        if result.get("success"):
            assert "code" in result, "Should contain generated code"
            assert (
                len(result.get("code", "")) > 100
            ), "Complex task should generate substantial code"
...
"""
        result = await self.call_tool(
            http_client, "review_code", {"code": code, "model": "starcoder"}
        )
        assert isinstance(result, dict), "Result should be a dictionary"
        assert (
            "success" in result
            or "quality_score" in result
            or "review_result" in result
        )
...
"""
        ...
        if "test_code" in result:
            ...
            assert (
                "test_" in test_code or "def test" in test_code
...
"""
        result = await self.call_tool(
            http_client, "analyze_complexity", {"code": code, "detailed": True}
        )
        assert isinstance(result, dict), "Result should be a dictionary"
        assert (
            "success" in result
            or "cyclomatic_complexity" in result
            or "complexity" in result
        )
    ...
    async def test_formalize_task(self, http_client):
        ...
        assert (
            "success" in result
            or "formalized_description" in result
            or "requirements" in result
        )
...
class TestToolChains:
    ...
    async def test_generate_review_chain(self, http_client):
        ...
        gen_result = await self.call_tool(
            ...
            "generate_code",
            {
                "description": "create a simple calculator with add and multiply",
                "model": "starcoder",
            },
        )
    ...
    async def test_generate_format_analyze_chain(self, http_client):
        ...
        # Format code
        format_result = await self.call_tool(http_client, "format_code", {"code": code})
        ...
        ), "Complexity result should be a dictionary"
    ...
    async def test_generate_tests_chain(self, http_client):
        ...
        gen_result = await self.call_tool(
            ...
            "generate_code",
            {
                "description": "create a simple function to check if a number is even",
                "model": "starcoder",
            },
        )
        ...
        test_result = await self.call_tool(
            http_client, "generate_tests", {"code": code, "test_framework": "pytest"}
        )
...
class TestConversationContext:
    ...
    async def test_context_chain_build_review_test(self, http_client):
        ...
        messages = [
            {
                ...
                "tool": "generate_code",
                "args": {
                    "description": "create a simple calculator with add function",
                    "model": "starcoder",
                },
            },
            {"type": "context", "tool": "review_code", "args": {"model": "starcoder"}},
            {
                "type": "context",
                "tool": "generate_tests",
        ...
        assert "code" in results[0] or results[0].get(
            "success", False
        ), "First step should generate code"
