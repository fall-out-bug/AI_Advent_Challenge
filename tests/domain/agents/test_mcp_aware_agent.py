...
class TestMCPAwareAgent:
    ...
    async def test_agent_initialization(self, mock_mcp_client, mock_llm_client):
        ...
        """
        # Arrange & Act
        agent = MCPAwareAgent(mcp_client=mock_mcp_client, llm_client=mock_llm_client)
    ...
    async def test_process_request_success(
        self, mock_mcp_client, mock_llm_client, mock_agent_request, mock_agent_response
    ):
    ...
    async def test_process_request_tool_selection(
        ...
        """
        # Arrange
        mock_mcp_client.discover_tools = AsyncMock(return_value=sample_tools_metadata)
        mock_llm_client.make_request = AsyncMock(
            return_value=ModelResponse(
    ...
    async def test_process_request_invalid_tool(
        self, mock_mcp_client, mock_llm_client, mock_agent_request
    ):
        ...
        assert (
            "nonexistent_tool" in response.error.lower()
