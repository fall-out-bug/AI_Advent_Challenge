...
class TestPerformanceMetrics:
    ...
    async def test_tool_discovery_latency(self):
        ...
        # Benchmarks from docs/reference/en/PERFORMANCE_BENCHMARKS.md (discovery p95 <~1.5s in CI)
        assert elapsed_time < DISCOVERY_LATENCY_THRESHOLD, (
            f"Discovery took {elapsed_time:.3f}s "
            f"(threshold {DISCOVERY_LATENCY_THRESHOLD:.2f}s)"
        )
        assert len(tools) > 0
    ...
    async def test_calculator_tool_latency(self):
        ...
        assert elapsed_time < CALCULATOR_LATENCY_THRESHOLD, (
            f"Tool call took {elapsed_time:.3f}s "
            f"(threshold {CALCULATOR_LATENCY_THRESHOLD:.2f}s)"
        )
    ...
    async def test_token_counting_performance(self):
        ...
        assert elapsed_small < TOKEN_LATENCY_THRESHOLDS["small"], (
            f"Small payload latency {elapsed_small:.3f}s "
            f"(threshold {TOKEN_LATENCY_THRESHOLDS['small']:.2f}s)"
        )
        ...
        assert elapsed_medium < TOKEN_LATENCY_THRESHOLDS["medium"], (
            f"Medium payload latency {elapsed_medium:.3f}s "
            f"(threshold {TOKEN_LATENCY_THRESHOLDS['medium']:.2f}s)"
        )
        ...
        assert elapsed_large < TOKEN_LATENCY_THRESHOLDS["large"], (
            f"Large payload latency {elapsed_large:.3f}s "
            f"(threshold {TOKEN_LATENCY_THRESHOLDS['large']:.2f}s)"
        )
...
class TestMemoryPerformance:
    ...
    async def test_large_text_handling(self):
        ...
        assert elapsed_time < LARGE_TEXT_LATENCY_THRESHOLD, (
            f"Large text processing took {elapsed_time:.3f}s "
            f"(threshold {LARGE_TEXT_LATENCY_THRESHOLD:.2f}s)"
        )
...
class TestScalability:
    ...
    async def test_model_listing_performance(self):
        ...
        assert elapsed_time < MODEL_LISTING_LATENCY_THRESHOLD, (
            f"Model listing took {elapsed_time:.3f}s "
            f"(threshold {MODEL_LISTING_LATENCY_THRESHOLD:.2f}s)"
        )
