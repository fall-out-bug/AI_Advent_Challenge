...
def build_session():
    ...
    spark = (
        SparkSession.builder.appName("hw3_load_to_redis")
...
def main():
    spark = build_session()
    try:
        df = spark.read.parquet(FEATURES_PATH).select(
            "user_id", "prev_count", "prev_avg", "prev_std", "recency"
        )
    ...
    if total_users == 0:
        raise RuntimeError(
            f"No test features to load (FEATURES_PATH={FEATURES_PATH}). Failing task."
        )
    # Пишем фичи в Redis
    ...
    def write_meta(_):
        ...
        r.hset(
            f"{KEY_PREFIX}:meta",
            mapping={"users": total_bc.value, "ts": int(time.time())},
        )
