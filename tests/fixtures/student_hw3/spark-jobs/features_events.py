...
def fe_train(spark):
    ...
    w_hist = Window.partitionBy("user_id").orderBy(F.col("ts").asc()).rowsBetween(Window.unboundedPreceding, -1)
    feats = (
        train
        .withColumn("prev_count", F.count("*").over(w_hist))
    ...
    feats = feats.withColumn("recency", (F.col("ts") - F.col("prev_last_ts")).cast("double"))
    # Fill cold-start rows where prev_count is null (no history)
    feats = feats.fillna({"prev_count": 0, "prev_avg": 0.0, "prev_std": 0.0, "recency": 1e9})
    out = feats.select("user_id","movie_id","ts","label","prev_count","prev_avg","prev_std","recency")
...
def fe_test(spark):
    ...
    test = test.withColumn("recency", (F.col("ts") - F.col("prev_last_ts")).cast("double"))
    test = test.fillna({"prev_count": 0, "prev_avg": 0.0, "prev_std": 0.0, "recency": 1e9})
    ...
    out = test.select("user_id","movie_id","ts","label","prev_count","prev_avg","prev_std","recency")
