...
def main():
    ...
    ratings = (
        spark.read.option("header", True).schema(schema).csv(ratings_path)
    ...
    df = df.join(last, on="user_id", how="left").withColumn("is_test", F.col("rn") == F.col("max_rn"))
