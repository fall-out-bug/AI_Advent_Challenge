...
def main():
    ...
    agg_basic = ratings.groupBy("user_id").agg(
    ...
    # профиль по жанрам
    joined = ratings.join(movies, on="movie_id", how="left").select(
        "user_id", "movie_id", "genres"
    )
    genre_arr = F.split(F.col("genres"), "\\|")
    exploded = (
        joined.withColumn("genre", F.explode(genre_arr))
    ...
    per_genre = (
        exploded.groupBy("user_id", "genre")
