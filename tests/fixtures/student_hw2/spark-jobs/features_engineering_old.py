...
SHUFFLE_PARTS = int(
    os.getenv("SPARK_SHUFFLE_PARTITIONS", "1024")
)  # было 256 → подняли до 1024
...
def main():
    ...
    agg_basic = (
        ratings.groupBy("user_id").agg(
            F.avg("rating").alias("avg_rating"),
            F.countDistinct("movie_id").alias("num_movies"),
            F.max("ts").alias("last_interaction_ts"),
            F.array_sort(F.collect_set(F.col("movie_id").cast("int"))).alias(
                "movie_ids"
            ),
        )
    ).persist(StorageLevel.MEMORY_AND_DISK)
    ...
    per_genre = (
        user_movie.join(genre_map, on="movie_id", how="inner")
    ...
    per_genre = per_genre.join(totals, on="user_id", how="left").withColumn(
        "pct", F.col("cnt") / F.col("total")
    )

    genre_profile = per_genre.groupBy("user_id").agg(
        F.map_from_arrays(
            F.collect_list("genre"), F.collect_list(F.round("pct", 6))
        ).alias("genre_profile")
    )
    ...
    features = features.withColumn(
        "genre_profile",
        F.when(F.col("genre_profile").isNull(), empty_map).otherwise(
            F.col("genre_profile")
        ),
    )
    ...
    (
        ...
        .coalesce(
            WRITE_PARTS
        )  # укрупняем файлы → меньше метаданных и шансов потерять блок
